...
+ (FMXModel *)modelWithResultSet:(FMResultSet *)rs
{
    ...
    for (id key in [columns keyEnumerator]) {
        ...
        id value = nil;
        id object = [rs objectForColumn:column.name];
...
-(void) createTable{
    FMXTableMap *table = [[FMXDatabaseManager sharedManager] tableForModel:[self class]];
    NSDictionary *columns = table.columns;
     if([self checkExitTable:table.tableName])
         return;
    NSMutableString *query = [[NSMutableString alloc] init];
    ...
    for (id key in [columns keyEnumerator]) {
        ...
        if([column.name isEqualToString:table.primaryKeyName]){
            ...
            if(!column.increments)
                ...
                [query appendFormat:@"%@ %@ not null, FOREIGN KEY( %@ ) REFERENCES %@( %@ )",column.name,[self convetFMXColumnType:column.type],column.name,column.nameTableLink,column.nameForeignKey];
            }else
    ...
    [query appendFormat:@")"];
    // create table
    if([[FMXDatabaseManager sharedManager]  openDatabase]){
        [[[FMXDatabaseManager sharedManager] getDatabase] executeUpdate:query];
        [[FMXDatabaseManager sharedManager] closeDataBase];
    }
}
-(BOOL) checkExitTable:(NSString*) nameTable{
    // SELECT name FROM sqlite_master WHERE type='table' AND name='{table_name}'
    if([[FMXDatabaseManager sharedManager]  openDatabase]){
        FMDatabase *db = [[FMXDatabaseManager sharedManager] getDatabase];
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT name FROM sqlite_master WHERE type='table' AND name='%@'",nameTable]];
        while ([rs next]) {
            [[FMXDatabaseManager sharedManager] closeDataBase];
